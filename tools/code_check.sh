...
# Example: echo "*:include/ignition/math/Vector2.hh:230" > $SUPPRESS
...
CPPCHECK_INCLUDES="-I ./include -I $builddir -I test -I ./include/ignition/sensors"
...
# cpplint
if [ $xmlout -eq 1 ]; then
  (echo $CPPLINT_FILES | xargs python tools/cpplint.p --extensions=cc,hh --headers=hh  --root=include --quiet 2>&1) \
    | python tools/cpplint_to_cppcheckxml.py 2> $xmldir/cpplint.xml
elif [ $QUICK_CHECK -eq 0 ]; then
  echo $CPPLINT_FILES | xargs python tools/cpplint.py --extensions=cc,hh --headers=hh --root=include --quiet 2>&1
fi
